...
class ListItem<T extends ListableItem> extends StatefulWidget {
  ...
  final Function(T listItem, dynamic newValue) updateFunction;
...
class _ListItemState<T extends ListableItem> extends State<ListItem<T>> {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          //? Right part
          widget.listItem is TodoList
            //? (TODOLIST) Back Button + 3-Dot-Menu
            ? todoListEnd(colors)
  ...
  Padding todoEnd(ColorScheme colors) {
    if (widget.listItem is Todo) {
      final todo = widget.listItem as Todo;

      return Padding(
        padding: const EdgeInsets.only(right: 15),
        child: IconButton(
          onPressed: () {
              widget.updateFunction(widget.listItem, !todo.completed);
          },
          icon: Icon(
  ...
  Row todoListEnd(ColorScheme colors) {
    return Row(
